...
class AreaLightTexture final : public Emitter<Float, Spectrum> {
public:
    MTS_IMPORT_BASE(Emitter, m_flags, m_shape, m_medium)
    MTS_IMPORT_TYPES(Scene, Shape, Texture)

    AreaLightTexture(const Properties &props) : Base(props) {
    ...
    Spectrum eval(const SurfaceInteraction3f &si, Mask active) const override {
        ...
        return select(
            ...
            unpolarized<Spectrum>(m_radiance->eval(si, active) * m_radiancetex->eval(si, active)),
            0.f
        );
    }
...
MTS_IMPLEMENT_CLASS_VARIANT(AreaLightTexture, Emitter)
MTS_EXPORT_PLUGIN(AreaLightTexture, "Area emitter with texture")
