...
.. _emitter-areatexture:

Area light with texture (:monosp:`areatexture`)
---------------------------
...
This plugin implements an area light with texture, i.e. a light source that emits
non-diffuse illumination from the exterior of an arbitrary shape.
The emission profile with regards to angle to the normal is specified via the nvalue parameter, forming a phong lobe.
Furthermore, since it occupies a nonzero amount of space, an
...
emitter shape and specify an :monosp:`areatexture` instance as its child:

.. code-block:: xml
    :name: sphere-light

    <shape type="sphere">
        <emitter type="areatexture">
...
class AreaLightTextureNondiffuse final : public Emitter<Float, Spectrum> {
public:
    MTS_IMPORT_BASE(Emitter, m_flags, m_shape, m_medium)
    MTS_IMPORT_TYPES(Scene, Shape, Texture)

    AreaLightTextureNondiffuse(const Properties &props) : Base(props) {
        ...
        m_radiancetex = props.texture<Texture>("radiancetex");
        m_nvalue = props.texture<Texture>("nvalue", Texture::D65(1.f));
    ...
    Spectrum eval(const SurfaceInteraction3f &si, Mask active) const override {
        ...
        Float cos_theta_val = Frame3f::cos_theta(si.wi);
        UnpolarizedSpectrum n_val = m_nvalue->eval(si, active);
        UnpolarizedSpectrum phong_lobe_val = m_radiance->eval(si, active) * m_radiancetex->eval(si, active);
        phong_lobe_val *= ((n_val + 2.f) / (2.f * math::Pi<Float>));
        phong_lobe_val *= (pow(cos_theta_val, n_val) / cos_theta_val);

        return select(
            Frame3f::cos_theta(si.wi) > 0.f,
            unpolarized<Spectrum>(phong_lobe_val),
            0.f
        );
    }
    ...
    std::pair<Ray3f, Spectrum> sample_ray(Float time, Float wavelength_sample,
        ...
        if (!m_radiancetex->is_spatially_varying()) {
            ...
            std::tie(si.uv, pdf) = m_radiancetex->sample_position(sample2, active);
            active &= neq(pdf, 0.f);
    ...
    sample_direction(const Interaction3f &it, const Point2f &sample, Mask active) const override {
        ...
        if (!m_radiancetex->is_spatially_varying()) {
            ...
            auto [uv, pdf] = m_radiancetex->sample_position(sample, active);
            //std::cout << uv << " " << pdf << std::endl;
            active &= neq(pdf, 0.f);
    ...
    Float pdf_direction(const Interaction3f &it, const DirectionSample3f &ds,
        ...
        if (!m_radiancetex->is_spatially_varying()) {
            ...
            value = m_radiancetex->pdf_position(ds.uv, active) * sqr(ds.dist) /
    ...
    std::string to_string() const override {
        ...
        oss << "AreaLightTextureNondiffuse[" << std::endl
...
private:
    ...
    ref<Texture> m_radiancetex;
    ref<Texture> m_nvalue;
};

MTS_IMPLEMENT_CLASS_VARIANT(AreaLightTextureNondiffuse, Emitter)
MTS_EXPORT_PLUGIN(AreaLightTextureNondiffuse, "Area emitter with texture and phong lobe")
